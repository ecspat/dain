...
var core_map = Array.prototype.map;
...
Observer.prototype.defineGetter = function(obj, prop, getter) {
	this.prop_writes.push({ obj: obj, prop: prop, val: getter, getter: true });
};

Observer.prototype.defineSetter = function(obj, prop, setter) {
	this.prop_writes.push({ obj: obj, prop: prop, val: setter, setter: true });
};
...
Observer.prototype.funcall = function(pos, callee, recv, args, kind) {
	...
	if (callee.getTag().type === 'clientobj') {
		this.callbacks.push({
			...
			args: [recv && recv.getTag() || this.tagLiteral(null, recv)].concat(core_map.call(args, function(v) {
				return v.getTag();
			})),
			kind: kind
		});
...
function filter_out_unknown(ary) {
	...
	function contains_unknown(obj) {
		if(obj && typeof obj === 'object') {
			if(obj === UNKNOWN) {
				...
				for(var p in obj) {
					if(obj.hasOwnProperty(p) && contains_unknown(obj[p])) {
						return true;
					}
				}
			}
	...
	for(var i=0,n=ary.length;i<n;++i) {
		if(!contains_unknown(ary[i])) {
			res[res.length] = ary[i];
		}
	}
	return res;
}
