...
var tagLiteral = Observer.prototype.tagLiteral = function(pos, lit) {
	...
	case 'object':
		...
		if(lit instanceof RegExp)
			return REGEXP;
		if(util.hasOwnProperty(lit, '__tag')) {
			tag = lit.__tag;
		} else if(!pos) {
			return UNKNOWN;
		} else {
...
Observer.prototype.defineGetter = function(obj, prop, getter) {
	...
	if(pw) {
		this.prop_writes[this.prop_writes.length] = pw;
	}
};
...
Observer.prototype.tagNativeArgument = function(callee, arg, idx) {
	if(arg && util.hasOwnProperty(arg, '__tag'))
		return arg.__tag;
		
	if (util.hasOwnProperty(callee, '__tag')) {
...
Observer.prototype.tagCallee = function(res) {
	...
	if(Object(res) === res) {
		...
		if(util.hasOwnProperty(res, '__tag'))
			return res.__tag;
			
		// maybe it is an instance of a function we know?
		var proto = Object.getPrototypeOf(res);
		if(proto && util.hasOwnProperty(proto, '__tag') && proto.__tag.default_proto_of) {
...
Observer.prototype.enterFunction = function(pos, fn) {
	this.current_fn[this.current_fn.length] = fn;
};

Observer.prototype.leaveFunction = function() {
	--this.current_fn.length;
};
...
Observer.prototype.returnFromFunction = function(retval) {
	if(retval.getValue() !== void(0)) {
		...
		if(ret) {
			this.returns[this.returns.length] = ret;
		}
	}
};
...
Observer.prototype.done = function() {
	var res = {
		...
		toString: function() {
			return '{\n' +
				   '  "prop_writes": [\n' + util.array_join(this.prop_writes, ',\n') + '\n  ],\n' +
				   '  "returns": [\n' + util.array_join(this.returns, ',\n') + '\n  ],\n' +
				   '  "callbacks": [\n' + util.array_join(this.callbacks, ',\n') + '\n  ]\n' +
				   '}';
		}
	};
	return res;
