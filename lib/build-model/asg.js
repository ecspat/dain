...
function unfold_asgs(decls) {
	...
	function recordDependencies(root) {
		...
		enter: function(node) {
			...
			encl_fun = node;
		    if(node.refers_to) {
			var relatum = node.refers_to.declarations[0].init;
			if(relatum && relatum.type === 'NewExpression' &&
			   relatum.callee.refers_to) {
			    var instantiated_fun = relatum.callee.refers_to.declarations[0].init;
			    if(instantiated_fun === encl_fun)
				return;
			}
